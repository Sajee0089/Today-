...
/* ════════════════════════════════════════════════
   DATA LOADING — tries direct browser, then Vercel proxy
   The proxy has a synthetic fallback so it ALWAYS returns data
════════════════════════════════════════════════ */
function timeout(ms) {
...
async function loadKlines(limit = 200) {
  // 1. Try direct browser fetch to Binance public REST (CORS-safe)
  const DIRECT = [
  ...
  for (const base of DIRECT) {
    try {
      const r = await tryFetch(base + "?symbol=" + SYMBOL + "&interval=" + INTERVAL + "&limit=" + limit, 5000);
      if (r && r.ok) {
        const d = await r.json();
        if (Array.isArray(d) && d.length > 0) {
          return { bars: d.map(parseBinanceKline), source: "binance-direct" };
        }
      }
    } catch (_) {}
  }

  // 2. Try CoinGecko directly from browser
  try {
    const r = await tryFetch(
      "https://api.coingecko.com/api/v3/coins/tether-gold/ohlc?vs_currency=usd&days=3",
      7000
    );
    if (r && r.ok) {
      const d = await r.json();
      if (Array.isArray(d) && d.length > 0) {
        return { bars: d.map(parseCGCandle), source: "coingecko-direct" };
      }
    }
  } catch (_) {}

  // 3. Try Vercel proxy (server-side, has its own fallback chain + synthetic)
  try {
    const r = await tryFetch("/api/klines?symbol=" + SYMBOL + "&interval=" + INTERVAL + "&limit=" + limit, 12000);
    if (r && r.ok) {
      const j = await r.json();
      if (j && j.data && j.data.length > 0) {
        return { bars: j.data.map(parseBinanceKline), source: j.source || "proxy" };
      }
    }
  } catch (_) {}

  // 4. Generate local synthetic data — last resort, chart ALWAYS shows
...
function parseCGCandle(r) {
  // [timestamp, open, high, low, close]
  return { o:+r[1], h:+r[2], l:+r[3], c:+r[4], v:500, t:r[0], done:true };
}
...
function makeSynthetic(n = 200) {
  ...
  for (let i = n - 1; i >= 0; i--) {
    const o = p;
    const m = (Math.random() - 0.498) * 4.5;
    const c = Math.max(3100, Math.min(3600, p + m));
    const w = Math.random() * 2.5;
    bars.push({ o: r2(o), h: r2(Math.max(o,c)+w), l: r2(Math.min(o,c)-w*0.6), c: r2(c), v:500, t: now - i*MS15, done:true });
    p = c;
  }
  return bars;
}
...
export default function App() {
  ...
  const loadCandles = useCallback(async()=>{
    ...
    if (result.source==="synthetic") {
      setDataErr("Using simulated data — all live sources unreachable");
      push("⚠ Using synthetic chart data (Binance/CoinGecko unreachable)");
    } else {
      push("✅ "+result.bars.length+" candles loaded from "+result.source+" · $"+result.bars[result.bars.length-1].c.toFixed(2));
    }
    setLoad(false);
  },[]);
  ...
  const connectWS = useCallback(()=>{
    ...
    ws.onmessage=ev=>{
      try{
        ...
        setBars(prev=>{
          // KEY FIX: always update/append regardless of prev.length
          let upd;
          if(prev.length===0){
            // No history yet — start building from this WS tick
            upd=[bar];
          } else {
            const last=prev[prev.length-1];
            if(last.t===bar.t){
              upd=[...prev.slice(0,-1),bar]; // update current candle
            } else if(k.x){
              upd=[...prev.slice(-299),bar]; // new closed candle
            } else {
              upd=[...prev.slice(0,-1),bar]; // update last with live tick
            }
          }
    ...
    <div style={F({background:"#020c18",minHeight:"100vh",color:"#e2e8f0",maxWidth:660,margin:"0 auto"})}>
      ...
      {tab==="chart"&&(
        <div style={{padding:"10px 14px",display:"flex",flexDirection:"column",gap:8}}>
          ...
          {dataErr&&<div style={{background:"#1c0707",border:"1px solid #991b1b",borderRadius:9,padding:"10px 13px",fontSize:10,color:"#fca5a5",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span>⚠ {dataErr}</span>
