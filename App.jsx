...
const INTERVAL    = "15m";
const REST_PUB    = "https://api.binance.com";         // public — CORS ok
const WS_STREAM   = "wss://stream.binance.com:9443/ws"; // public WS — no CORS
...
/* PUBLIC REST (direct, no auth) */
async function fetchKlines(limit = 200) {
  const r = await fetch(`${REST_PUB}/api/v3/klines?symbol=${SYMBOL}&interval=${INTERVAL}&limit=${limit}`);
...
export default function App() {
  ...
  const connectWS = useCallback(()=>{
    ...
    setWsS("connecting");
    const ws = new WebSocket(`${WS_STREAM}/${SYMBOL.toLowerCase()}@kline_${INTERVAL}`);
    wsRef.current=ws;
    let ping=null;

    ws.onopen=()=>{
      setWsS("live");
      push("🟢 Binance WebSocket live — real-time XAUUSDT 15M");
      ping=setInterval(()=>ws.readyState===1&&ws.send(JSON.stringify({method:"ping"})),20000);
    };
    ...
    ws.onerror=()=>setWsS("error");
    ws.onclose=e=>{
      clearInterval(ping);
      if(wsRef.current===ws){
        setWsS("disconnected");
        if(e.code!==1000){
          push("🔄 WS disconnected — reconnecting in 5s…");
          reconnR.current=setTimeout(connectWS,5000);
        }
      }
    };
  },[]);

  const disconnectWS=()=>{ clearTimeout(reconnR.current); wsRef.current?.close(1000); wsRef.current=null; setWsS("off"); };
    ...
    <div style={F({background:"#020c18",minHeight:"100vh",color:"#e2e8f0",maxWidth:660,margin:"0 auto"})}>
      ...
      {tab==="chart"&&(
        <div style={{padding:"10px 14px",display:"flex",flexDirection:"column",gap:8}}>
          ...
          {dataErr&&<div style={{background:"#1c0707",border:"1px solid #991b1b",borderRadius:9,padding:"10px",fontSize:10,color:"#fca5a5"}}>{dataErr}</div>}
          {bars.length>0?(<>
